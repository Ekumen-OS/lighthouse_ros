...
#include <lighthouse_ros_msgs/msg/lighthouse_station_map.hpp>
#include <lighthouse_ros_msgs/msg/sensor_measurements_stamped.hpp>
#include <lighthouse_sensor_model/lighthouse_sensor_model.hpp>
#include <visualization_msgs/msg/marker_array.hpp>

namespace lighthouse_amcl {

class LighthouseMCLFilter {
public:
  using Particle = std::tuple<Sophus::SE2d, beluga::Weight>;

  explicit LighthouseMCLFilter(beluga::LighthouseStationMapType &&stations_map)
      : motion_model_(motion_model_params()),
        uniform_state_distribution_{Eigen::AlignedBox2d(
            Eigen::Vector2d{-10., -10.}, Eigen::Vector2d{10., 10.})},
        sensor_model_(sensor_model_params(), std::move(stations_map)) {
    ...
    update_policy_ = beluga::make_policy(
        [=](const Sophus::SE2d &odometry,
            const beluga::LighthouseDirectionalDetection &detections) mutable {
          return motion_policy(odometry);
        });
    resample_policy_ = beluga::policies::every_n(kResampleInterval);
  }

  bool update(Sophus::SE2d odometry,
              beluga::LighthouseDirectionalDetection detections) {
    if (particles_.empty()) {
      return false;
    }
...
private:
  ...
  beluga::any_policy<Sophus::SE2d, beluga::LighthouseDirectionalDetection>
      update_policy_;
  ...
  beluga::DifferentialDriveModel motion_model_;

  beluga::LighthouseSensorModel2D sensor_model_;
  ...
  static beluga::LighthouseSensorModelParam sensor_model_params() {
    auto params = beluga::LighthouseSensorModelParam{};
    params.sigma_angular_error = 0.01;
    params.sensor_height = 0.00;
    return params;
  }
};

class LighthouseMCLNode : public rclcpp::Node {
public:
  explicit LighthouseMCLNode(const rclcpp::NodeOptions &options)
      : rclcpp::Node("lighthouse_mcl_node", options) {
    {
    ...
    {
      auto callback = std::bind(&LighthouseMCLNode::timer_callback, this);
      timer_ = create_wall_timer(std::chrono::milliseconds(200),
                                 std::move(callback), common_callback_group);
    }

    {
      auto callback = std::bind(&LighthouseMCLNode::initial_pose_callback, this,
                                std::placeholders::_1);
    ...
    {
      auto callback = std::bind(&LighthouseMCLNode::map_callback, this,
                                std::placeholders::_1);
      map_sub_ = create_subscription<LighthouseStationMap>(
          "lighthouse_map", rclcpp::SystemDefaultsQoS(), std::move(callback),
          common_subscription_options);

      RCLCPP_INFO(get_logger(), "Subscribed to %s topic",
                  map_sub_->get_topic_name());
    }

    {
      auto callback = std::bind(&LighthouseMCLNode::feature_detections_callback,
                                this, std::placeholders::_1);
      feature_detections_sub_ = create_subscription<LighthouseDetections>(
          "lighthouse", rclcpp::SensorDataQoS(), std::move(callback),
          common_subscription_options);

      RCLCPP_INFO(get_logger(), "Subscribed to %s topic",
                  feature_detections_sub_->get_topic_name());
    }

    particle_cloud_pub_ = create_publisher<PoseArray>(       //
        "particle_cloud",                                    //
...
private:
  ...
  using LighthouseDetections =
      lighthouse_ros_msgs::msg::SensorMeasurementsStamped;
  using PoseWithCovarianceStamped =
      geometry_msgs::msg::PoseWithCovarianceStamped;
  using LighthouseStationMap = lighthouse_ros_msgs::msg::LighthouseStationMap;

  void feature_detections_callback(
      const LighthouseDetections::ConstSharedPtr &features) {
    if (!particle_filter_) {
    ...
    beluga::LighthouseDirectionalDetection detections;
    detections.base_station_id = features->sweep_data.base_station_id;
    for (std::size_t sensor_id = 0; sensor_id < 4; ++sensor_id) {
      auto &deck_sensor_angles = detections.deck_sensor_angles[sensor_id];
      deck_sensor_angles.azimuth =
          features->sweep_data.sensor_angles[sensor_id].azimuth;
      deck_sensor_angles.elevation =
          features->sweep_data.sensor_angles[sensor_id].elevation;
    }
    ...
    if (did_update) {
      ...
      RCLCPP_INFO(
          get_logger(),
          "Particle filter update iteration stats: %ld particles "
          "- %.3fms",
  ...
  void map_callback(const LighthouseStationMap::ConstSharedPtr &station_map) {
    beluga::LighthouseStationMapType internal_station_map;
    for (const auto &station : station_map->station_poses) {
      const auto station_pose_in_world = Sophus::SE3d{
          Sophus::SO3d{Eigen::Quaterniond{
              station.pose.orientation.w, station.pose.orientation.x,
              station.pose.orientation.y, station.pose.orientation.z}},
          Eigen::Vector3d{station.pose.position.x, station.pose.position.y,
                          station.pose.position.z}};
      internal_station_map.station_poses_in_world[station.base_station_id] =
          station_pose_in_world;
    }

    static auto initial_guess =
        std::make_pair(Sophus::SE2d{Sophus::SO2d{}, Eigen::Vector2d{0., 0.}},
    ...
    particle_filter_ =
        std::make_unique<LighthouseMCLFilter>(std::move(internal_station_map));
  ...
  std::unique_ptr<LighthouseMCLFilter> particle_filter_;
  ...
  rclcpp::Subscription<LighthouseDetections>::SharedPtr feature_detections_sub_;
  rclcpp::Subscription<PoseWithCovarianceStamped>::SharedPtr initial_pose_sub_;
  rclcpp::Subscription<LighthouseStationMap>::SharedPtr map_sub_;
};

} // namespace lighthouse_amcl

#include "rclcpp_components/register_node_macro.hpp"
RCLCPP_COMPONENTS_REGISTER_NODE(lighthouse_amcl::LighthouseMCLNode)
